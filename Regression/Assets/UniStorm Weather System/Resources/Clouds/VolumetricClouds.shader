Shader "UniStorm/Clouds/Volumetric"
{
    ...
    fixed4 frag(v2f i) : SV_Target
    {
		...
		if (_UseUniStormFog == 1)
		{
			...
			half3 c_test = min(pow(max(0, dot(rd, normalize(_SunVector.xyz))), _SunAlpha) * _SunBeta, 1) * clamp(_SunControl, 0.0, 1.0) ;
			...
			if (_EnableDithering == 1)
			{
				...
				c.rgb = float4(((AtmosphericFinal.rgb) + (c_sun * _SunIntensity) + (c_moon * _MoonIntensity) + (getNoise(i.uv) * 1.5) - 0.0025), 0);
...
Pass
{
	...
	#include "UnityCG.cginc"
        ...
        half4 frag(v2f i) : SV_Target
        {
			...
			{
				float converganceSpeed = lerp(0.9, 0.99, ms01);
				return lerp(prevSample, lerp(currSample, prevSample, correct), lerp(converganceSpeed, lerp(0.15, 0.25, ms01) * 2, luvN));
			}
#else
