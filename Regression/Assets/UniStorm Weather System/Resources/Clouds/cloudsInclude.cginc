...
// Square UVs provided in -1,-1 to 1,1 range
...
// Circle UVs provided in -1,-1 to 1,1 range
...
#else
    m = smoothstep(0.0f, lerp(2.5f, _uCloudsBaseEdgeSoftness, fade), m + (lerp(_uCloudsCoverage + _uCloudsCoverageBias - 1, _uCloudsCoverage + _uCloudsCoverageBias, fade) - 1.));
#endif
...
float4 renderCloudsInternal(float3 ro, float3 rd, inout float dist)
{
    ...
    int nSteps = lerp(10, CLOUD_MARCH_STEPS, dot(rd, float3(0, 1, 0)*DISTANT_CLOUD_MARCH_STEPS)); // 10 samples for the distant clouds
#elif defined(MEDIUM)
    ...
    for (int s = 0; s < nSteps; s++)
    {
        ...
        if (alpha > 0.f) 
        {
			float3 detail2 = cloudMapDetail(p * 0.25, norY, 0.25);
            dist = min(dist, d);
            float3 ambientLight = lerp(
                lerp(_uCloudsAmbientColorBottom - detail2.r * (lerp(0.2, 0.05, (_uCloudsCoverage)) * _uAttenuation * 0.4f), 0.0f, saturate(_uLightning * 3.0f)),
                lerp(_uCloudsAmbientColorTop - detail2.r * (0.1 * _uAttenuation * 0.9), _uLightningColor * 3.0f, saturate(_uLightning * 10.0f)),
                norY) * _uCloudsColor;
...
#else
			float3 detail = cloudMapDetail(p * _uCloudNoiseScale, norY, 0.5);
			float3 light = _uSunColor * _uAttenuation * 1.5 * smoothstep(0.02, 0.2, sundotup);
#if UNITY_COLORSPACE_GAMMA
			light *= smoothstep(-0.03f, 0.075f, sundotup) - detail.r * clamp(lerp(1.3, 2, norY), 1.1, 1.4);
#else
			light *= smoothstep(-0.03f, 0.075f, sundotup) - clamp(detail.r * 1.3, 0, 1.1);
#endif
            
			//Smooth opposite clouds
			light *= lerp(smoothstep(0.9f, 0.4f, sundotrd), 1.0f, smoothstep(0.1, 0.75f, sundotup));
